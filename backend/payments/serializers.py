...
class GiftSerializer(serializers.ModelSerializer):
    gift_icon = serializers.SerializerMethodField()
    gift_animation_type = serializers.SerializerMethodField()
    class Meta:
        model = Gift
        fields = ["id", "name", "coins", "value_etb", "gift_icon", "gift_animation_type"]

    def get_gift_icon(self, obj):
        icon, _ = map_gift_animation(str(obj.name))
        return icon

    def get_gift_animation_type(self, obj):
        _, anim = map_gift_animation(str(obj.name))
        return anim


# Helper mapping function used by serializers and views
def map_gift_animation(name: str):
    key = (name or '').lower()
    ...
    mapping = [
        ...
        ("rose", ("🌹", "rose_bloom")),
        ("heart-shaped chocolate", ("🍫❤️", "chocolate_hearts")),
        ("chocolate", ("🍫❤️", "chocolate_hearts")),
        ("teddy", ("🧸", "teddy_wave")),
        ("romantic song", ("🎶", "music_notes")),
        ("song", ("🎶", "music_notes")),
        ("candlelight dinner", ("🍷✨", "wine_clink")),
        ("bouquet", ("💐", "roses_rain")),
        ("photo frame", ("🖼️", "photo_frame")),
        ("kiss", ("💋", "kiss_blow")),
        ("perfume", ("🧴", "perfume_spray")),
        ("ring", ("💍", "ring_sparkle")),
        ("weekend getaway", ("✈️💕", "plane_hearts")),
        ("car ride", ("🚗💖", "car_hearts")),
        ("home", ("🏡❤️", "home_glow")),
    ]
    for substr, pair in mapping:
        if substr in key:
            return pair
    # Fallbacks
    return ("🎁", "grow_fade")
